...
public class SqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>,
			...
			for (String packageToScan : typeAliasPackageArray) {
				...
				try {
					Resource[] metaInfResources = resourcePatternResolver
							.getResources(packageToScan);
					for (Resource r : metaInfResources) {

						//将resource路径转换成包路径
						String packageString = getPackageString(r.getURL()
								.toString());
						...
						if (logger.isDebugEnabled()) {
							logger.debug("Scanned package: '" + packageString
									+ "' for aliases");
						}
					}
				} catch (Exception e) {
	 ...
	 * 将resource路径转换成包路径
	 */
	private String getPackageString(String urlStr) {
		if (urlStr != null) {
			
			//形如 xxx/WEB-INF/classes/com/module/test/model/
			int index = urlStr.lastIndexOf(CLASSES_PREFIX);
			if (index > -1) {
				//截取从 classes/ 到末尾的 / 之间的字符
				String str = urlStr.substring(index + 8, urlStr.length() - 1);
